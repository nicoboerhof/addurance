    ...
    (function () {
      ...
      function resize() {
        ...
        // Tijdens gameplay: gebruik container.clientHeight zodat top-positie (pixels)
        // en schaalberekening exact op elkaar aansluiten — geen gap boven/onder canvas.
        // container.clientHeight = 100dvh (door onze CSS fix), vv.height kan hier licht van afwijken.
        const containerH = container.clientHeight || (vv ? vv.height - navOffset : null) || window.innerHeight - navOffset;
        const ch = isGamePlaying
          ? containerH
          : (vv ? vv.height - navOffset : null) || containerH || window.innerHeight - navOffset;

        // 🔥 KEY FIX: altijd volledig in beeld (no cropping ever)
        const scaleX = cw / GAME_W;
        const scaleY = ch / GAME_H;
        const scale = Math.max(0.1, Math.min(scaleX, scaleY));

        // 🔥 Center + scale — gebruik pixels voor top zodat percentage-resolutie geen rol speelt
        canvas.style.transform = `translate(-50%, -50%) scale(${scale})`;
        canvas.style.transformOrigin = 'center center';
        canvas.style.position = 'absolute';
        canvas.style.left = '50%';
        canvas.style.top = Math.round(containerH / 2) + 'px';

        // Player reset blijft hetzelfde
        const aspect = (autoBgImg.complete && autoBgImg.naturalWidth > 0)
          ? autoBgImg.naturalWidth / autoBgImg.naturalHeight
          : 1.648;
    ...
    function goToStart() {
      if (raf) { cancelAnimationFrame(raf); raf = null; }
        ...
        document.documentElement.style.overflow = ''; // desktop fix
      ...
      function startGame() {
        ...
        // Block scroll during game — position:fixed fixes iOS Safari (overflow:hidden alone doesn't work)
